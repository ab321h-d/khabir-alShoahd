...
const completenessStatusLabels: Record<CompletenessStatus, string> = { incomplete: "غير مكتمل", needs_review: "يحتاج مراجعة", complete: "مكتمل" };
...
export default function Director() {
  ...
  /**
   * R-NEXT-3: تعبئة تلقائية لرقم واتساب المعلم — لا تكتب فوق رقم موجود
   * بالفعل في السجل الحالي. المصدر: أولًا Teacher Contact المحفوظ، وإلا
   * أحدث رقم صالح من سجلات استيراد سابقة لنفس الاسم المُطبَّع. تُحدَّث حالة
   * الواجهة المحلية فقط هنا — الحفظ الفعلي في IndexedDB يحدث فقط عند تفاعل
   * المدير مع الحقل (onBlur)، لا كتابة صامتة في الخلفية.
   */
  useEffect(() => {
    if (!selected || (selected.whatsappNumber || "").trim().length > 0) return;
    let live = true;
    void (async () => {
      // PHASE ID-3E: teacherId ثابت متاح -> مسار الهوية حصريًا، بلا أي
      // fallback للاسم إطلاقًا (لو غاب سجل الهوية، الحقل يبقى فارغًا عمدًا
      // — لا إعادة إنتاج لمشكلة تصادم الاسم التي صُمِّم هذا المسار لحلها).
      if (selected.teacherId) {
        const derived = await directorStore.getTeacherContactByIdentity(selected.teacherId);
        if (!live || !derived) return;
        setSubmissions((items) => items.map((item) => item.id === selected.id && !(item.whatsappNumber || "").trim() ? { ...item, whatsappNumber: derived } : item));
        return;
      }
      // Legacy — بلا teacherId: نفس المسار القديم بالاسم، بلا أي تغيير.
      const normalizedName = normalizeTeacherName(selected.teacherName);
      if (!normalizedName) return;
      const fromContact = await directorStore.getTeacherContact(normalizedName);
      const derived = fromContact || await directorStore.findLatestWhatsAppNumberForTeacher(normalizedName);
      if (!live || !derived) return;
      setSubmissions((items) => items.map((item) => item.id === selected.id && !(item.whatsappNumber || "").trim() ? { ...item, whatsappNumber: derived } : item));
    })();
    return () => { live = false; };
  }, [selected?.id, selected?.teacherName]);
  ...
  const importPdf = async () => {
    ...
    const isPlainPdf = file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf");
    if (!isZipPackage && !isPlainPdf) return showToast("يرجى اختيار ملف PDF أو حزمة .khabir.zip فقط");
    setWorking(true);
    try {
      let pdfFile = file;
      ...
      if (isZipPackage) {
        ...
        try {
          ...
          showToast("تعذر فتح الحزمة، الملف تالف");
          setWorking(false);
          return;
        }
        const pdfBytes = unzipped["portfolio.pdf"];
        if (!pdfBytes) {
          showToast("الحزمة لا تحتوي ملف أداء صالحًا");
          setWorking(false);
          return;
        }
        pdfFile = new File([pdfBytes], file.name.replace(/\.zip$/i, "").replace(/\.khabir$/i, "") || "portfolio.pdf", { type: "application/pdf" });
        const metadataBytes = unzipped["completeness.json"];
        if (metadataBytes) {
          try {
            completenessMetadata = validateCompletenessMetadata(JSON.parse(new TextDecoder().decode(metadataBytes)));
          } catch {
  ...
  const updateReview = async (patch: Partial<Pick<DirectorSubmission, "reviewStatus" | "reviewLevel" | "academicTerm" | "comment" | "whatsappNumber" | "whatsappMessage">>) => {
    ...
    try {
      ...
      if (next) setSubmissions((items) => items.map((item) => {
        if (item.id !== next.id) return item;
        // لا يقصد هذا التعديل تغيير whatsappNumber، وقاعدة البيانات أعادت قيمة
        // فارغة، لكن الواجهة تحمل رقمًا مُعبَّأ تلقائيًا لم يُكتَب بعد — يُحافَظ
        // عليه بدل مسحه، تمامًا كما لو لم يحدث الاستبدال إطلاقًا لهذا الحقل.
        const shouldPreserveAutofill = !("whatsappNumber" in enrichedPatch) && !(next.whatsappNumber || "").trim() && (item.whatsappNumber || "").trim().length > 0;
        return shouldPreserveAutofill ? { ...next, whatsappNumber: item.whatsappNumber } : next;
      }));
    } catch {
